    ...
    useEffect(() => {
        ...
        if (sessionId) {
            fetch(`/api/get-session?session_id=${sessionId}`)
                ...
                setError("Não foi possível carregar as informações da barbearia. Por favor, tente novamente mais tarde.");
                setIsLoading(false);
            });
        }
    }, [searchParams]);
      ...
      <div className="mx-auto max-w-2xl rounded-lg bg-white p-8 shadow-md dark:bg-gray-800">
        ...
        {barbershopInfo && (<>
            ...
            <div className="mb-6 rounded-lg bg-yellow-100 p-4 text-yellow-700">
              ...
              <p>
                Guarde o ID da sua barbearia em um local seguro. Você precisará
                dele para acessar e gerenciar sua barbearia no futuro.
              </p>
            </div>
          </>)}
        <p className="mb-8 text-center text-gray-600 dark:text-gray-300">
          Você pode gerenciar sua barbearia e modificar suas informações na tela
          de gerenciamento.
        </p>
        <div className="flex justify-center">
          <Button onClick={() => barbershopInfo &&
            router.push(`/manage-barbershop/${barbershopInfo.id}`)} className="hover:bg-primary-dark bg-primary text-white">
