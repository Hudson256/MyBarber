...
export async function getBarbershopTimes({ barbershopId, date, }) {
    try {
        const dayOfWeek = date.getDay();
        console.log(`Fetching times for barbershop ${barbershopId} on day ${dayOfWeek}`);
        ...
        if (barbershopAvailability.length === 0) {
            console.log(`No availability found for barbershop ${barbershopId}. Adding default hours.`);
        ...
        console.log(`Found ${barbershopAvailability.length} availabilities:`, barbershopAvailability);
        ...
        const bookedTimes = bookings.map((booking) => booking.date.toLocaleTimeString("pt-BR", {
            hour: "2-digit",
            minute: "2-digit",
        }));
