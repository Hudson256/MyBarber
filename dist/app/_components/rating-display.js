...
const RatingDisplay = ({ ratings, services }) => {
    ...
    const renderStars = (rating) => {
        const stars = [];
        for (let i = 1; i <= 5; i++) {
            stars.push(<StarIcon key={i} className={`h-5 w-5 ${i <= rating ? "text-yellow-500" : "text-gray-300"}`}/>);
        }
        return stars;
    };
    ...
    return (<div className="space-y-4 p-5">
      ...
      {ratings.map((rating, index) => {
            ...
            return (<div key={`${rating.userName}-${index}`} className="flex rounded-lg border border-gray-200 p-4 shadow-sm">
            <div className="mr-4 flex-shrink-0">
              <img src={rating.userAvatar || "/default-avatar.png"} // Caminho corrigido
             alt={`${rating.userName}'s avatar`} className="h-12 w-12 rounded-full object-cover" width={48} height={48}/>
            </div>
            <div className="flex-grow">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  {rating.barberName && (<p className="text-sm text-gray-500">
